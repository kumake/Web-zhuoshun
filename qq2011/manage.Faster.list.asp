...
<%
	...
	if action<>"" and action="del" then
		''删除信息
		Con.Execute("delete * from Sp_FasterMenu where ID="&id&"")
		ALert "删除成功","manage.Faster.list.asp"
	end if	
%>
...
<DIV id=wrap>
	<UL id=tags>
	  <LI class=selectTag style="LEFT: 0px; TOP: 0px"><A onfocus="this.blur()" onclick="selectTag('tagContent0',this)" href="javascript:void(0)">快捷菜单管理</A></LI>
	</UL>
	<DIV id=tagContent>
		...
		<DIV>
			<TABLE cellSpacing="1" width="100%">
			  <TR class="content-td2">
			    ...
			    <TH width="25%" align="left">菜单名称</TH>
			    <TH width="55%" align="left">菜单URL</TH>
			  ...
			  <%
				...
				if total = 0 then 
					Dim Tarray:Tarray = Con.QueryData("select count(*) as total from Sp_FasterMenu")
				...
				Dim sql:sql = Con.QueryPageByNum("*","Sp_FasterMenu", "", "ID", true,current_page,pagesize)
				'response.Write sql
				'response.End()
			  ...
			  <TR class="content-td1">
				<TD>&nbsp;</TD>
				<TD><%=rs("MenuName")%></TD>
				<TD><a href="<%=rs("MenuUrl")%>"><%=rs("MenuUrl")%></a></TD>
				<TD><a href="manage.Faster.edit.asp?ID=<%=rs("ID")%>">修改</a>&nbsp;<a href="javascript:if(confirm('Sp_CMS系统提示:\r\n\n确认删除?删除后不可恢复!'))location.href='?action=del&id=<%=rs("ID")%>';">删除</a></TD>
			  </TR>
			  <%
			   rs.movenext
			   loop
