...
<%
	...
	if action<>"" and action="save" then 
		...
		'''''''
		Con.execute("update Sp_orderDeliver set deliverytype='"&deliverytype&"',Delivermoney='"&Delivermoney&"',[memo]='"&memo&"' where ID="&ID&" ")
		Alert "修改配送方式成功","pulg.order.deliver.list.asp"
	end if
	if ID=0 then
		Alert "参数传递失败",""
	else
		set rs = Con.Query("select * from Sp_orderDeliver where ID="&ID&"")
		...
		<DIV>
			<TABLE cellSpacing="1" width="100%">
			  <TR class="content-td1">
				<TD width="15%">配送方式：</TD>
				<TD><input name="txt_deliverytype" class="input" type="text" value="<%=rs("deliverytype")%>"><span class="huitext">&nbsp;必填</span></TD>
			  </TR>
			  <TR class="content-td1">
			    <td>配送费用：</td>
				<TD colspan="2"><input name="txt_Delivermoney" class="input" type="text" value="<%=rs("Delivermoney")%>"><span class="huitext">&nbsp;必填</span></TD>
			  </TR>
			  <TR class="content-td1">
			    <td>说明：</td>
				<TD colspan="2"><textarea name="txt_memo" cols="80" rows="8"><%=rs("memo")%></textarea></TD>
			  </TR>
			</TABLE>
		</DIV>
		<div class="divpadding">
		  <input name="btnsearch" value="修改配送方式" class="button" type="submit">
		</div>
	</DIV>
	</DIV>
</DIV>
