...
<DIV id=wrap>
	...
	<DIV class="tagContent selectTag content" id="tagContent0">
		...
		<DIV>
			<TABLE cellSpacing="1" width="100%">
			  <TR class="content-td2">
			    ...
			    <TH width="10%" align="left">用户组名称</TH>
			    <TH width="10%" align="left">用户名称</TH>
			    <TH width="25%" align="left">用户email</TH>
			  ...
			  <TR class="content-td1">
				...
				<TD><%if rs("isactive")=1 then:response.Write "激活":else:response.Write "<span class='redtext'>禁止</span>":end if%>&nbsp;<span class="huitext"><%if rs("isactive")=1 then:response.Write "<a href='?action=prohibit&ItemID="&rs("ID")&"' class='huitext'>禁止</a>":else:response.Write "<a href='?action=active&ItemID="&rs("ID")&"' class='huitext'>激活</a>":end if%></span></TD>
				<TD><a href="pulg.user.view.asp?GroupID=<%=rs("GroupID")%>&ItemID=<%=rs("ID")%>">详细</a>&nbsp;<a href="javascript:if(confirm('Sp_CMS系统提示:\r\n\n确认删除?删除后不可恢复!'))location.href='?action=del&ItemID=<%=rs("ID")%>';">删除</a></TD>
			  </TR>
			  <%
			   rs.movenext
			   loop
		...
		<div class="divpadding">
		  <input name="btnsearch" value="会员自动升级" class="button" type="button" onClick="javascript:location.href='pulg.user.upgrade.asp';">
		</div>
	</DIV>
	</DIV>
</DIV>
