...
<%
	...
	if action<>"" and action="save" then 
		...
		'response.End()
		Dim sql:sql = "update Sp_Manage set username='"&username&"',userRole='"&userRole&"'"
		if userpwd<>"" then
			sql =sql &" ,userpwd='"&MD5(userpwd)&"'"
		end if
		sql =sql &" where ID="&ID&""
		Con.execute(sql)
		Alert "修改管理员成功","manage.Group.User.list.asp"
	end if
	if ID=0 then
		Alert "参数传递失败",""
	else
		set rs = Con.Query("select * from Sp_Manage where ID="&ID&"")
		...
		<DIV>
			<TABLE cellSpacing="1" width="100%">
			  <TR class="content-td1">
				...
				<TD><input name="txt_username" class="input" type="text" value="<%=rs("username")%>"><span class="huitext">&nbsp;必填</span></TD>
			  </TR>
			  <TR class="content-td1">
				<TD width="15%">管理员密码：</TD>
				<TD><input name="txt_userpwd" class="input" type="text"></TD>
			  </TR>
			  <TR class="content-td1">
			  ...
			  <TR class="content-td1">
				...
				%>
				<input type="checkbox" name="txt_userRole" value="<%=RoleArray(0,j)%>" <%if instr(rs("userRole"),RoleArray(0,j))>0 or trim(rs("userRole"))=trim(RoleArray(0,i)) then response.Write "checked"%>>&nbsp;<%=RoleArray(1,j)%>&nbsp;&nbsp;
				<%
				next
				%>
				</TD>
