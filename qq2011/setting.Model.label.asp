...
<%
	...
	if action<>"" and action="del" then
		''删除信息
		Con.Execute("delete * from sp_Label where ID="&ItemID&"")
		ALert "删除成功","setting.Model.label.asp"
	end if	
%>
...
<DIV id=wrap>
	<UL id=tags>
	  <LI class=selectTag style="LEFT: 0px; TOP: 0px"><A onfocus="this.blur()" onclick="selectTag('tagContent0',this)" href="javascript:void(0)">标签管理</A></LI>
	  <LI><A href="setting.Model.label.add.asp">增加标签</A></LI>
	</UL>
	<DIV id=tagContent>
		...
		<DIV>
			<TABLE cellSpacing="1" width="100%">
			  ...
			  <%
				...
				if total = 0 then 
					Dim Tarray:Tarray = Con.QueryData("select count(*) as total from sp_Label where "&strwhere&"")
				...
				Dim sql:sql = Con.QueryPageByNum("id,name","sp_Label", ""&strwhere&"", "ID", false,current_page,pagesize)
				'response.Write sql
				'response.End()
			  ...
			  <TR class="content-td1">
				...
				<TD><%=rs("name")%></TD>
				<TD><a href="setting.Model.label.edit.asp?ItemID=<%=rs("ID")%>">修改</a>&nbsp;<a href="javascript:if(confirm('Sp_CMS系统提示:\r\n\n确认删除?删除后不可恢复!'))location.href='?action=del&ItemID=<%=rs("ID")%>';">删除</a></TD>
			  </TR>
			  <%
			   rs.movenext
			   loop
