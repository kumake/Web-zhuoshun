...
<%
	...
	if ModelID=0 then
		...
		if ubound(Modelarray,1)=0 then 
			'HtmlAlert "出现错误,请在模型字段中添加该模型的字段!至少添加一个字段#<a href='setting.Model.Field.add.asp'>增加字段</a>"
			'response.End()
...
<DIV id=wrap>
		...
		<DIV>
			...
			<TABLE cellSpacing="1" width="100%">
			  ...
			  <TR class="content-td1">
				...
				<input name="txt_RelationTitle" value="<%if ParentModelItemID<>0 then response.Write "已经选择"%>" disabled class="input" size="80" type="text">&nbsp;&nbsp;<a href="#" onClick="javascript:SelectTemplate('Model_Parent.asp?ModelID=<%=ParentModelID%>&value=txt_SubModelItemID&text=txt_RelationTitle');">选择</a>
				</TD>
			  </TR>
			  <%
			  end if
			  ...
			  <TR class="content-td1">
				<TD>标题：</TD>
				<TD><input name="txt_title" class="input" size="80" type="text"><span class="huitext">&nbsp;必填</span></TD>
			  </TR>
			  <%
			  ...
			  <TR class="content-td1">
				...
				<TD><input name="txt_title_En" class="input" size="80" type="text"><span class="huitext">&nbsp;必填</span></TD>
			  </TR>
			  <%end if%>
			  <TR class="content-td1" id="JumpUrl" style="display:none;">
				<TD>转向链接地址：</TD>
				<TD><input name="txt_jumpUrl" class="input" size="80" type="text">&nbsp;<font color="#ff0000">如果文章是转向链接，那么以下各项填写无效,请不要填写!</font></TD>
			  </TR>
			  <%
			  if ubound(Modelarray,1)=0 then
			  else
			  ...
			  <TR class="content-td1">
				<TD>属性:</TD>
				<TD>
				<input name="IsNew" type="checkbox" value="1">最新&nbsp;&nbsp;
			  ...
			  <TR class="content-td1">
				<TD>显示顺序</TD>
				<TD><input name="txt_IndexID" class="input" value="0" type="text"><span class="huitext">&nbsp;数值越大越靠前,相同时候其他参数其作用.</span></TD>
			  </TR>
			  <TR class="content-td1">
				<TD>最后更新时间</TD>
	...
	<div class="divpadding">
	  <input name="btnsearch" value="增加信息" class="button" type="submit">
	</div>
	</DIV>
</DIV>
</form>
...
<script language=javascript>
    ...
    function SelectTemplate(url)
    {
        ...
        g_pop.setContent("title","Sp_CMS窗口");
