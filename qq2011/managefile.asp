...
FileLoginName="7a57a5a743894a0e1"	'登陆用户名md5+"1"
FileLoginPwd="7a57a5a743894a0e2"	'登陆密码md5+"2"
...
<div id="index_bottom"><A href="http://www.cqqzx.com" target=""_blank"">SpCMS网络</a> 版权所有</div>
</div>
</body>
</html>
<%
...
sub LoginCheck()
	...
	loginname=F.ReplaceFormText("loginname")
	loginpwd=F.ReplaceFormText("loginpwd")
...
Sub RenameFolder
	...
	Else
		...
		NewFolderName=F.ReplaceFormText("NewFolderName")
		sFolder=F.ReplaceFormText("folder")
...
Sub RenameFile
	...
	Else
		...
		NewFileName=F.ReplaceFormText("NewFileName")
		Sfile=F.ReplaceFormText("filename")
		if right(FullsPath,1)="/" then slashvalue="" else slashvalue="/"
...
Sub CreateFolder
	If FullsPath="/" or FullsPath="./" Then
			sFile = FullsPath & F.ReplaceFormText("file")
			sFolder = FullsPath & F.ReplaceFormText("folder")
	Else
			sFile = FullsPath & "/" & F.ReplaceFormText("file")
			sFolder = FullsPath & "/" & F.ReplaceFormText("folder")
	End If
...
Sub SaveFile
	...
	session("foldername")=sPath
	'保存为新的文件
	if F.ReplaceFormText("NewFileName")<>"" Then
		Dim NewFileName,slashvalue,textStreamObject,filestuff,NewPathFileName
		NewFileName=F.ReplaceFormText("NewFileName")
		spath=request("path")
		if right(FullsPath,1)="/" then slashvalue="" else slashvalue="/"
		filestuff=F.ReplaceFormText("filestuff")
		...
		If Request.Querystring("overwrite") = "yes" Then
			...
			WriteFile.Write F.ReplaceFormText("filestuff")
			WriteFile.Close
			Response.Redirect("" & Session("lastpage") & "")
		...
		Else
			...
			If fs.FileExists(server.mappath(sFile)) Then
				...
				response.Write("<div id=""body_content"">")
				response.Write("<form name=""reform1"" action="""& scriptname &"?action=savefile&overwrite=yes&path="& spath &"&file=" & F.ReplaceFormText("File") & """ method=""post""><ul>")
				Response.Write "<LI><strong>替换现有的文件吗?</strong></a>"
				Response.Write "<LI><a href=""javascript:reform1.submit();"">覆盖此文件</a> &nbsp; <a href=""javascript:history.back()"">返回上一步</a><input type=""hidden"" value="""& F.ReplaceFormText("newfilestuff") &""" name=""filestuff""></LI>"
			...
			Else 
				...
				WriteFile.Write F.ReplaceFormText("newfilestuff")
				WriteFile.Close
