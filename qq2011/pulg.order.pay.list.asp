...
<%
	...
	if action<>"" and action="del" then
		''删除信息
		Con.Execute("delete * from Sp_orderPay where ID="&ItemID&"")
		ALert "删除成功","pulg.order.pay.list.asp"
	elseif action="batchdel" then
		Dim batchdelItemID:batchdelItemID = F.ReplaceFormText("ItemID")
		Con.Execute("delete * from Sp_orderPay where ID in ("&batchdelItemID&")")
		Alert "批量删除成功","pulg.order.pay.list.asp"
	end if	
%>
...
<DIV id=wrap>
	<UL id=tags>
	  <LI class=selectTag style="LEFT: 0px; TOP: 0px"><A onfocus="this.blur()" onclick="selectTag('tagContent0',this)" href="javascript:void(0)">支付方式管理</A></LI>
	  <LI><A href="pulg.order.pay.add.asp">增加支付方式</A></LI>
	</UL>
	<DIV id=tagContent>
		...
		<DIV>
			<TABLE cellSpacing="1" width="100%">
			  <TR class="content-td2">
			    ...
			    <TH width="30%" align="left">支付方式 </TH>
			  ...
			  <%
				...
				if total = 0 then 
					Dim Tarray:Tarray = Con.QueryData("select count(*) as total from Sp_orderPay where "&strwhere&"")
				...
				Dim sql:sql = Con.QueryPageByNum("*","Sp_orderPay", ""&strwhere&"", "ID", false,current_page,pagesize)
				'response.Write sql
				'response.End()
			  ...
			  <TR class="content-td1">
				...
				<td><%=rs("paytype")%></td>
				<TD><%=rs("memo")%></TD>
				<TD><a href="pulg.order.pay.edit.asp?ID=<%=rs("id")%>">修改</a>&nbsp;<a href="javascript:if(confirm('Sp_CMS系统提示:\r\n\n确认删除?删除后不可恢复!'))location.href='?action=del&ItemID=<%=rs("ID")%>';">删除</a></TD>
			  </TR>
			  <%
			   rs.movenext
			   loop
