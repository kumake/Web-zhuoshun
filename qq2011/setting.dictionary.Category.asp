...
<%
	...
	if action<>"" and action="del" then
		''删除信息
		Con.Execute("delete * from Sp_dictionaryCategory where ID="&ItemID&"")
		Con.Execute("delete * from Sp_dictionary where categoryID="&ItemID&"")		
		ALert "删除数据字典类别成功","setting.dictionary.Category.asp"
	end if	
%>
		...
		<DIV>
			<TABLE cellSpacing="1" width="100%">
			  <TR class="content-td2">
			    ...
			    <TH width="65%" align="left">类别名称</TH>
			  ...
			  <%
				...
				if total = 0 then 
					Dim Tarray:Tarray = Con.QueryData("select count(*) as total from Sp_dictionaryCategory")
				...
				Dim sql:sql = Con.QueryPageByNum("*","Sp_dictionaryCategory", "", "ID", true,current_page,pagesize)
				'response.Write sql
				'response.End()
			  ...
			  <TR class="content-td1">
				...
				<TD><a href="setting.dictionary.asp?CategoryID=<%=rs("ID")%>"><%=rs("dictionary")%></a></TD>
				<TD>
				<a href="setting.dictionary.add.asp?CategoryID=<%=rs("ID")%>" target=main>增加字典</a>&nbsp;<a href="setting.dictionary.asp?CategoryID=<%=rs("ID")%>">子类别</a>&nbsp;
				<%
				if rs("issystem")<>1 then
				%>
				<a href="setting.dictionary.Category.edit.asp?ItemID=<%=rs("ID")%>">修改</a>&nbsp;<a href="javascript:if(confirm('Sp_CMS系统提示:\r\n\n确认删除?删除后不可恢复!'))location.href='?action=del&ItemID=<%=rs("ID")%>';">删除</a>
				<%end if%>
				</TD>
			  </TR>
			  <%
