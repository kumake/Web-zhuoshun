...
<%
	...
	if action<>"" and action="del" then
		''删除信息
		Con.Execute("delete * from Sp_ShopCart where ID="&ItemID&"")
		ALert "删除成功","pulg.order.cart.list.asp"
	elseif action="batchdel" then
		Dim batchdelItemID:batchdelItemID = F.ReplaceFormText("ItemID")
		Con.Execute("delete * from Sp_ShopCart where ID in ("&batchdelItemID&")")
		''''''
		Alert "批量删除成功","pulg.order.cart.list.asp"
	end if	
%>
...
<DIV id=wrap>
	<UL id=tags>
	  <LI class=selectTag style="LEFT: 0px; TOP: 0px"><A onfocus="this.blur()" onclick="selectTag('tagContent0',this)" href="javascript:void(0)">购物车管理</A></LI>
	</UL>
	<DIV id=tagContent>
		...
		<DIV>
			<TABLE cellSpacing="1" width="100%">
			  <TR class="content-td2">
			    ...
			    <TH width="10%" align="left">购物车编号</TH>
				<TH width="32%" align="left">产品名称</TH>
			    <TH width="10%" align="left">产品价格</TH>
			    <TH width="5%" align="left">产品数量</TH>
			    <TH width="13%" align="left">小记</TH>
			  ...
			  <%
				...
				Dim strwhere:strwhere = " p.id=s.proid"
				if total = 0 then 
					Dim Tarray:Tarray = Con.QueryData("select count(*) as total from Sp_ShopCart S,user_pro P where "&strwhere&"")
				...
				Dim sql:sql = Con.QueryPageByNum("S.*,P.title","Sp_ShopCart S,user_pro P", ""&strwhere&"", "S.ID", false,current_page,pagesize)
				'response.Write sql
				'response.End()
			  ...
			  <TR class="content-td1">
				...
				<td><a href="pulg.user.view.asp?GroupID=1&ItemID=<%=rs("sessionID")%>"><%=rs("sessionID")%></a></td>
				<TD><%=rs("title")%></TD>
				<TD><%=rs("price")%></TD>
				<TD><%=rs("num")%></TD>
				<TD><%=rs("totalprice")%></TD>
