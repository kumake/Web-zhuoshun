...
<%
	...
	if action<>"" and action="save" then 	
		...
		Con.execute("update sp_Label set name='"&Labname&"',code='"&Labcontent&"' where ID="&ItemID&"")
		Alert "修改标签成功","setting.Model.label.asp"
	end if
	if ItemID = 0 then
		Alert "参数传递失败",""
	else
		set rs = Con.Query("select * from sp_Label where ID="&ItemID&"")
