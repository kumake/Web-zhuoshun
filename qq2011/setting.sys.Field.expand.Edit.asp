...
<%
	...
	if ID<>"" and isnumeric(ID) then
		FieldArray = Con.QueryRow("select T.tablename,F.ID,F.tableid,F.fieldname,F.FieldUI,F.FieldDescription,F.fieldtype,F.fieldlength,F.isNull,F.Attribute,F.defaultValue,T.ID from Sp_TableField F,Sp_Table T where F.TableID=T.ID and F.ID="&ID&"",0)
	...
	if action<>"" and action="save" then
	  ...
	  Dim defaultValue:defaultValue = F.ReplaceFormText("txt_defaultValue")
	  	'''增加记录
		Con.Execute("update Sp_tableField  set FieldUI ='"&FieldUI&"',FieldDescription='"&FieldDescription&"',isNull="&isNull&",Attribute='"&Attribute&"',defaultValue='"&defaultValue&"' where ID="&FieldArray(1)&"")
		'''增加物理表结构
		''''
		Alert "修改表单字段成功","setting.sys.Field.expand.asp?tableID="&FieldArray(2)&""
	end if
%>
...
<DIV id=wrap>
	<UL id=tags>
	  <LI class=selectTag style="LEFT: 0px; TOP: 0px"><A onfocus="this.blur()" onclick="selectTag('tagContent0',this)" href="javascript:void(0)">修改系统表字段扩展</A></LI>
	</UL>
	<DIV id=tagContent>
