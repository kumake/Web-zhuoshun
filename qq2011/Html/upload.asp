...
<input name="txt_uploadpassword" type="text" size="6" maxlength="6" class="input"><input type=file name=uploadfile size=1 style="width:80%" onchange="originalfile.value=this.value">
...
{
	/////验证上传密码
	if(<%=config_UploadPassword%>!=document.myform.txt_uploadpassword.value)
	{
		alert("SPCMS提醒您\n\n您输入的上传密码不正确！");
		return false;
	}
...
Sub DoRemote()
	Dim sContent, i
	For i = 1 To F.ReplaceFormText("eWebEditor_UploadText").Count 
		sContent = sContent & F.ReplaceFormText("eWebEditor_UploadText")(i) 
	Next
	If sAllowExt <> "" Then
