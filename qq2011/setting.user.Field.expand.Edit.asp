...
<%
	...
	if ID<>"" and isnumeric(ID) then
		FieldArray = Con.QueryRow("select M.UserGroup,F.ID,F.GroupID,F.fieldname,F.FieldUI,F.FieldDescription,F.fieldtype,F.fieldlength,F.isNull,F.Attribute,F.defaultValue,M.ID from Sp_UserExpField F,Sp_UserGroup M where F.GroupID=M.ID and F.ID="&ID&"",0)
	...
	if action<>"" and action="save" then
	  ...
	  Dim defaultValue:defaultValue = F.ReplaceFormText("txt_defaultValue")
	  	'''增加记录
		Con.Execute("update Sp_UserExpField  set FieldUI ='"&FieldUI&"',FieldDescription='"&FieldDescription&"',isNull="&isNull&",Attribute='"&Attribute&"',defaultValue='"&defaultValue&"' where ID="&FieldArray(1)&"")
		'''增加物理表结构
		''''
		Alert "修改表单字段成功","setting.user.Field.expand.asp?GroupID="&FieldArray(11)&""
	end if
%>
...
<DIV id=wrap>
	<UL id=tags>
	  <LI class=selectTag style="LEFT: 0px; TOP: 0px"><A onfocus="this.blur()" onclick="selectTag('tagContent0',this)" href="javascript:void(0)">修改用户字段扩展</A></LI>
	</UL>
	<DIV id=tagContent>
		...
		<DIV>
			<TABLE cellSpacing="1" width="100%">
			  <TR class="content-td1">
				<TD width="15%">模型名称：</TD>
				<TD>
				<select name="txt_GroupID" disabled>
				<%
					...
					response.Write ">"&rs("UserGroup")&"</option>"
					rs.movenext
					loop
					end if
				%>
